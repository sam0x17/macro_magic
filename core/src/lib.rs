...
use syn::{
    ...
    Ident, Item, ItemFn, Pat, Path, Result, Token, Visibility,
};
...
pub struct AttrItemWithExtra {
    ...
    _comma1: Comma,
    pub extra1: TokenTree2,
    _comma2: Comma,
    pub extra2: TokenTree2,
    _comma3: Comma,
    pub extra3: TokenTree2,
}
...
pub fn forward_tokens_internal<T: Into<TokenStream2>>(tokens: T) -> Result<TokenStream2> {
    ...
    match (args.extra1, args.extra2, args.extra3) {
        (Some(extra1), Some(extra2), Some(extra3)) => Ok(quote! {
            #source_path! {
                #target_path,
...
pub fn forward_tokens_inner_internal<T: Into<TokenStream2>>(tokens: T) -> Result<TokenStream2> {
    ...
    match (parsed.extra1, parsed.extra2, parsed.extra3) {
        ...
        pub #orig_sig {
            pub #inner_sig {
                ...
                let #tokens_ident: proc_macro::TokenStream =
                    if let #mm_path::__private::TokenTree2::Group(group) = __combined_args.extra1 {
                        group.stream().into()
                    } else {
                        unreachable!("Macro magic internal error: group token is expected")
                    };

                let __source_path: proc_macro::TokenStream =
                    if let #mm_path::__private::TokenTree2::Group(group) = __combined_args.extra2 {
                        group.stream().into()
                    } else {
                        unreachable!("Macro magic internal error: group token is expected")
                    };

                let __custom_tokens: proc_macro::TokenStream =
                    if let #mm_path::__private::TokenTree2::Group(group) = __combined_args.extra3 {
                        group.stream().into()
                    } else {
                        unreachable!("Macro magic internal error: group token is expected")
                    };
